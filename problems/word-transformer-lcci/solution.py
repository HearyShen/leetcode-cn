...
class Solution:
    def findLadders(self, beginWord: str, endWord: str, wordList: List[str]) -> List[str]:
        ...
        self.rearchedWords.add(beginWord)
        while len(bfs_queue) > 0:
            ...
            if cur_word == endWord:
                print(f"BFS search count: {bfs_count}.")
                return cur_path

            nextStepWords = self.findNextWords(cur_word, endWord, words) - self.rearchedWords
            for nextWord in nextStepWords:
                nextPath = cur_path.copy()
                nextPath.append(nextWord)
                bfs_queue.append(nextPath)

            self.rearchedWords = self.rearchedWords.union(nextStepWords)
    ...
    def __init__(self):
        self.nextWords = dict()     # {key: wordGenericForm, value: words}
        self.rearchedWords = set()  # rearched words shouldn't be searched again
    ...
    def charDiffCount(srcWord: str, destWord: str) -> int:
        """Returns True if srcWord and destWord have only one char in difference."""
        if not (len(srcWord) == len(destWord)):
            return len(srcWord)
        return sum([int(not srcWord[i] == destWord[i]) for i in range(len(srcWord))])
